...
int main()
{
    ...
    while (cin >> width) {
        ...
        vector< vector<int> > detected_image;
        ...
        output_width.push_back(width);
        output_pairs.push_back(pairs);
    }
    
    for (int i = 0; i != output_width.size(); ++i) {
        cout << output_width[i] << endl;
        for (int j = 0; j != output_pairs[i].size(); ++j) {
            cout << output_pairs[i][j].first << ' ' << output_pairs[i][j].second << endl;
        }
    }
    cout << '0' << endl;
...
vector< vector<int> >& constuct_image(vector< vector<int> >& image, int width, vector<int>& facilitate)
{
    ...
    while (cin >> pixel.first >> pixel.second) {
        ...
        if (pixel.second > 5 * width) {
            int top = width - column;
            int down = (pixel.second - top) % width;
            int remain = pixel.second - top - down - 3 * width;
