...
vector<string> detect_similarity(const vector<string>& dictionary, const string& word)
{
    ...
    for (vector<string>::size_type i = 0; i != dictionary.size(); ++i) {
        if (word.size() == 1 && dictionary[i].size() == 1) {
        ...
        bool deleting = false;
        if (word.size() + 1 == dictionary[i].size()) {
            inserting = true;
        }
        if (word.size() == dictionary[i].size() + 1) {
            deleting = true;
        }
        ...
        if ((similarity + 1 == dictionary[i].size() && dictionary[i].size() == word.size())
            || (similarity + 1 == dictionary[i].size() && dictionary[i].size() == word.size() + 1 && inserting)
            || (similarity + 1 == word.size() && dictionary[i].size() + 1 == word.size() && deleting))
        {
            result.push_back(dictionary[i]);
        }
    }
